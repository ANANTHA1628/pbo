...
public class EventBackend {
    ...
    public ArrayList<ComboItem> getVenueList() {
        ...
        try {
            Connection c = Koneksi.getKoneksi(); // Sesuaikan nama class koneksi Anda
    ...
    public boolean isNamaAda(String nama, int idKecuali) {
        try {
            ...
            // Jika ID > 0, berarti Update (cek semua KECUALI id diri sendiri)
            if (idKecuali == -1) {
            ...
            ps.setString(1, nama);
            if (idKecuali != -1) ps.setInt(2, idKecuali);
            
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return rs.getInt(1) > 0; // True jika ada duplikat
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<Object[]> getEventList(String keyword) {
        ...
        try {
            ...
            while (rs.next()) {
                data.add(new Object[]{
                    rs.getInt("id"), rs.getString("nama_event"), rs.getString("tanggal"),
                    rs.getLong("pool_prize"), rs.getLong("harga_registrasi"), rs.getString("nama_venue")
                });
            }
        } catch (Exception e) {
            e.printStackTrace();
